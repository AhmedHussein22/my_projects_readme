# 🏡 Bait AlWatan - بيت الوطن

Real estate platform for Egyptian expatriates featuring land information, project news, and property listings.

---

[![Flutter](https://img.shields.io/badge/Flutter-%5E3.0-blue?logo=flutter)](#)
[![Platforms](https://img.shields.io/badge/Platforms-Android%20%7C%20iOS-lightgrey)](#)
[![License](https://img.shields.io/badge/License-MIT-green)](#)
[![Firebase](https://img.shields.io/badge/Firebase-%5E9.0-yellow?logo=firebase)](#)

---

## 📸 Screenshots

<p align="center">
  <img width="1920" height="960" alt="1" src="https://github.com/user-attachments/assets/92aec805-b67e-4b48-a955-e9401d7dc289" />
 <img width="1920" height="1266" alt="4" src="https://github.com/user-attachments/assets/2cd63eb4-2d20-46e5-9ab4-34cd68c82d85" />
<img width="1920" height="1266" alt="3" src="https://github.com/user-attachments/assets/d614b2bb-9bde-40de-a5bb-acea6eeed487" />
<img width="1920" height="1266" alt="2" src="https://github.com/user-attachments/assets/ed7b8d0e-55b5-410c-aea0-f594c7fde0cd" />

</p>

---

## ✅ Features

- ✓ Browse land and property information
- ✓ Real estate project news feed
- ✓ Properties for sale listings
- ✓ Google Maps integration
- ✓ REST API integration
- ✓ Push notifications (FCM)
- ✓ Image caching
- ✓ Multi-language support
- ✓ Responsive UI

---

## 🧰 Technology Stack

- Flutter (Mobile)
- Dart
- Firebase (Firestore, Auth, Storage, Messaging)
- Provider (State Management)
- Google Maps API
- REST API

<p align="left">
  <img alt="Flutter" src="https://img.shields.io/badge/Flutter-%5E3.0-blue?logo=flutter" height="24" />
  <img alt="Dart" src="https://img.shields.io/badge/Dart-%5E2.0-blue?logo=dart" height="24" />
  <img alt="Firebase" src="https://img.shields.io/badge/Firebase-%5E9.0-yellow?logo=firebase" height="24" />
  <img alt="Provider" src="https://img.shields.io/badge/Provider-state_management-lightgrey" height="24" />
</p>

---

## 🏛 Architecture

Provider-based MVVM:

- **Model:** Property data, news entities
- **View:** Flutter widgets, maps integration
- **ViewModel:** Business logic, API calls
- **Services:** REST API, notifications, maps

---

## 📦 Key Packages Used

- provider — state management
- firebase_core, cloud_firestore, firebase_auth, firebase_storage, firebase_messaging — Firebase
- google_maps_flutter — maps
- dio — HTTP client
- cached_network_image — image caching
- intl — localization

---

## 📲 Store Links

- **Play Store:** https://play.google.com/store/apps/details?id=com.icloud.bait_watan

---

## 🧾 License

This project is released under the MIT License.

---

## ✉️ Contact / Links

- **Name:** Ahmed Hussein
- **Email:** ahmed.dev229@gmail.com
- **LinkedIn:** https://www.linkedin.com/in/ahmed-hussein-66b1b71a5/
- **GitHub:** https://github.com/AhmedHussein22
- **Portfolio:** https://ahmed-portfolio-4ccaa.web.app/

---

Thank you for checking out Bait AlWatan! 🏡
